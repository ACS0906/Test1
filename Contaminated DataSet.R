...
conta=matrix(rep(round(2*as.numeric(apply(D,2,max))+rnorm(9,0,1)),5),5)
...
X<-DD[,c(2,4,8)]
nDD=cbind(X,Y)
#################@@@ Robust Correlation Making ############
DataR=cov2cor(MeanVar(nDD,Beta=0.1)$V)
r11=DataR[1:3,1:3]
r22=DataR[4:6,4:6]
r12=DataR[1:3,4:6]
r21=DataR[4:6,1:3]
...
L;PV;df
Table=round(data.frame(CanCorr=can.corr,Chisquare=L,dd=df,p.value=PV),3)
################## canonical weights 
a<-sr11%*%sd$u
a[,colSums(a)<0]=-a[,colSums(a)<0]
b<-sr22%*%sd$v
